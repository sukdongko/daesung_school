...
Begin VB.Form INT010 
   Caption         =   "입학사정 >> 입학원서 출력 >> 종합 입학원서 출력"
   ClientHeight    =   11010
   ClientLeft      =   3195
   ClientTop       =   1530
   ClientWidth     =   14520
...
'>> 학년별 내역
    Select Case Trim(basModule.SchCD)
        ...
        Case "N"
            OPTIONS(0) = "▶인문계 학생들은 사회탐구중 4과목까지 선택할 수 있으며, 제2외국어는 1과목을 선택할 수 있습니다."
            ...
            Labels(78).Caption = "▼수능 전문반은 신청자가 적을 경우 반을 구성하지 못할 수도 있습니다."
            Labels(1).Caption = ""
        ...
        Case "K", "W", "Q", "J", "B"
            OPTIONS(0) = "▶인문계 사회선택은 구사를 포함하여 4과목을 선택하십시요. 세계지리, 경제지리는 극소수일 경우성반되지 않을 수 있습니다."
            OPTIONS(1) = "▶자연계 수리영역 선택과목 중 확률통계는 극소수일 경우 성반되지 않을 수 있습니다."
            OPTIONS(2) = ""
        ...
        Case "S"
            ...
            OPTIONS(1) = "▶제2외국어 시간에 제2외국어를 하지 않는 학생들을 위해 언어, 수리, 영어, 세계사, 세계지리를 동시간대에 편성하니 1과목을 선택하시기 바랍니다."
        ...
        Case "M"
            OPTIONS(0) = "▶인문계 사회선택은 구사를 포함하여 4과목을 선택하십시요. 세계지리, 경제지리는 극소수일 경우 성반되지 않을 수 있습니다."
            OPTIONS(1) = "▶자연계 수리영역 선택과목 중 확률통계는 극소수일 경우 성반되지 않을 수 있습니다."
            OPTIONS(2) = ""
...
Private Sub Get_STD_Data()
    ...
    Select Case Trim(basModule.SchCD)
        Case "S"
            sStr = sStr & " DECODE(GR,'1','수능등급','2','2007 평가','','') AS GR, "
        Case "P"
            sStr = sStr & " DECODE(GR,'8','수능등급','9','2007 평가','6','3등급','','') AS GR, "
        Case Else
            sStr = sStr & " '' AS GR, "
    End Select
    ...
    sStr = sStr & "    FROM ( "
            ...
            sStr = sStr & "         MAX(PHOTO_PATH) AS PHOTO_PATH , MAX(R_WAY     ) AS R_WAY      , MAX(PTS_SEL   ) AS PTS_SEL   , MAX(ORD_NO    ) AS ORD_NO    , MAX(PTS_SEL) AS GR "
    ...
    With uSTD(Index)
        ...
        Select Case Trim(.kaeyol)   '<< 계열: 01,02,03-인문,자연,예체   06,05-수능인문,자연  06,07 -강남법대,의대
            ...
            Case "23":        Select Case Trim(basModule.SchCD)
                                     Case "B":  접수계열.Text = "인 문 계"
                                                접수계열2.Text = "인문PS"
                              End Select
                                          
            Case "24":        Select Case Trim(basModule.SchCD)
                                     Case "B":  접수계열.Text = "자 연 계"
                                                접수계열2.Text = "자연PM"
                              End Select
