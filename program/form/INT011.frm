   ...
   Begin MSComctlLib.ImageList imgList 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "INT011.frx":0150
            Key             =   ""
         EndProperty
      EndProperty
   End
End
Attribute VB_Name = "INT011"
...
'>> 학년별 내역
    ...
    Select Case Trim(basModule.schcd)
        ...
        Case "N"
            OPTIONS(0) = "● 국어, 수학, 영어, 과목 중 (심화)또는(기본) 수업 1과목을 선택해야 하며, 탐구과목 4과목 중 1과목을 선택할 수 있습니다."
            OPTIONS(1) = "● 인문계는 생활과 윤리, 윤리와 사상, 세계지리, 동아시아사, 세계사, 경제, 제2외국어, 자연계는 과학Ⅱ(4과목)는 재수 정규반부터 수업합니다."
            OPTIONS(2) = "● 반당 수강생 수 증감에 따라 분반 또는 합반할 수 있습니다."
            OPTIONS(3) = "● 인문계는(국어B, 수학A, 영어B) / 자연계(국어A, 수학B, 영어B형)으로 수업합니다."
        ...
        Case "S"
            OPTIONS(0) = "● 국어, 수학, 영어, 과목 중 (심화)또는(기본) 수업 2과목을 선택해야 하며, 탐구과목중 1과목을 선택할 수 있습니다."
            OPTIONS(1) = "● 인문계는 생활과 윤리, 윤리와 사상, 세계지리, 동아시아사, 세계사, 경제, 제2외국어, 자연계는 과학Ⅱ(4과목)는 재수 정규반부터 수업합니다."
            OPTIONS(2) = "● 반당 수강생 수 증감에 따라 분반 또는 합반할 수 있습니다."
            OPTIONS(3) = "● 인문계는(국어B, 수학A, 영어B) / 자연계(국어A, 수학B, 영어B형)으로 수업합니다."
...
Private Sub Get_STD_Data()
    ...
    sStr = sStr & "         SEL1       , SEL2       , SEL3       , SEL4       , SEL5       ,SEL7        ,"
        ...
        ElseIf .RecordCount > 0 Then
            ...
            For nRec = 1 To .RecordCount Step 1
                ...
                sTmp = "":      If IsNull(.Fields("SEL5")) = False Then sTmp = .Fields("SEL5")
                    uSTD(nRec).SEL5 = sTmp
                sTmp = "":      If IsNull(.Fields("SEL7")) = False Then sTmp = .Fields("SEL7") ' 클리닉
                    uSTD(nRec).SEL7 = sTmp
...
Private Sub Std_Data_Show(Index As Long)
    ...
    With uSTD(Index)
        ...
        If basModule.schcd = "N" Then
            test1.Text = Get_StrGwaMokRange(.SEL7, CLINIC_L_CLASS, CLINIC_L_CLASS + (CLINIC_L_COUNT - 1)) ' 국어
            test2.Text = Get_StrGwaMokRange(.SEL7, CLINIC_M_CLASS, CLINIC_M_CLASS + (CLINIC_M_COUNT - 1))  ' 수학
            test3.Text = Get_StrGwaMokRange(.SEL7, CLINIC_E_CLASS, CLINIC_E_CLASS + (CLINIC_E_COUNT - 1))  ' 영어
        ElseIf basModule.schcd = "S" Then
            test1.Text = Get_StrGwaMokRange(.SEL7, CLINIC_L_CLASS, CLINIC_L_CLASS + (CLINIC_L_COUNT - 1)) ' 국어
            test2.Text = Get_StrGwaMokRange(.SEL7, CLINIC_M_CLASS, CLINIC_M_CLASS + (CLINIC_M_COUNT - 1))  ' 수학
            test3.Text = Get_StrGwaMokRange(.SEL7, CLINIC_E_CLASS, CLINIC_E_CLASS + (CLINIC_E_COUNT - 1))  ' 영어
        End If
        ...
        test4.Text = ""
        test4.Text = Div_Gwamok_NM("SEL1", .SEL1) '사탐/과탐
        
        test4.Text = test4.Text & Div_Gwamok_NM("SEL3", .SEL3)
        
        txtMu_Type.Text = IIf(.MU_TYPE = "", "수능등급", .MU_TYPE)
        ...
        Select Case Trim(.EXMTYPE)
            Case "0"
                ...
                '내신등급이면 등급 표시 아니면 합계 표시 (황혜영차장님 요청)
                If .MU_TYPE = "내신등급" Then
                    합계.Text = .N_NUM
                Else
                    합계.Text = CInt(.K_NUM) + CInt(.M_NUM) + CInt(.E_NUM)
                End If
