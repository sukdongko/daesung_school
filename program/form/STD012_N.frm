...
Begin VB.Form STD012_N 
   Caption         =   "입학사정 >> 학생전체 조회 (차수별)(노량진,송파)"
   ClientHeight    =   12960
   ClientLeft      =   6045
   ClientTop       =   1770
   ClientWidth     =   15450
   ...
   Begin VB.Frame Frame20 
      ...
      Begin VB.Frame Frame21 
         ...
         Begin FPSpread.vaSpread sprExcel_STD_Data 
            ...
            EndProperty
            MaxCols         =   16
            SpreadDesigner  =   "STD012_N.frx":76CA
         End
...
'###############################################################################################################################################
    ...
    Select Case Trim(Right(cboPassCN, 30))
        Case "ALL"      ' /* 합격생은 모두 조회 */
            ...
            sStr = sStr & "                      (NVL(K_NUM,0)+NVL(M_NUM,0)+NVL(E_NUM,0)+NVL(T_NUM,0)) AS TOT_NUM ,"
...
'            sStr = sStr & " DECODE(PTS_SEL,'8','수능등급','9','2007 평가','6','3등급','','') AS 구분, "
    ...
    sStr = sStr & "    FROM ( "
            ...
            sStr = sStr & "         MAX(K_NUM     ) AS K_NUM      , MAX(M_NUM     ) AS M_NUM      , MAX(E_NUM     ) AS E_NUM     , MAX(T_NUM     ) AS T_NUM     , MAX(TOT_NUM   ) AS TOT_NUM    , MAX(N_NUM   ) AS N_NUM   ,"
