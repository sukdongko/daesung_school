...
Begin VB.Form INT020 
   Caption         =   "입학사정 >> 입학원서 출력 >> 강남 입학원서 출력"
   ClientHeight    =   10710
   ClientLeft      =   2415
   ClientTop       =   3030
   ClientWidth     =   14190
   ...
   Begin VB.PictureBox pReportControl 
      ...
      Begin VB.PictureBox pReportViewer 
         ...
         Begin VB.TextBox 학교석차 
            ...
            Left            =   7800
            TabIndex        =   25
            Text            =   "영어(999/999), 수학(999/999)"
            Top             =   6990
            Width           =   5000
         End
         Begin VB.TextBox 문과선택과목 
            Appearance      =   0  '평면
            BorderStyle     =   0  '없음
            BeginProperty Font 
               Name            =   "굴림체"
   ...
   Begin MSComctlLib.ImageList imgList 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "INT020.frx":1406
            Key             =   ""
         EndProperty
      EndProperty
   End
End
Attribute VB_Name = "INT020"
...
Private Sub cmdFind_Click()
    ...
    Select Case Trim(basModule.SchCD)
        ...
        Case Else
            MsgBox "강남대성학원이 아닌 경우 출력물이 다를 수 있습니다.", vbExclamation + vbOKOnly, "학생조회"
    End Select
...
Private Sub Get_STD_Data()
    ...
    sStr = sStr & "         SCORE1, SCORE2, SCORE3, T_RANK, ALL_CNT, GTOT, "
    ...
    'sStr = sStr & "                 HOPE_ACACD, SEX, NVL(KEYOL,'1') AS KEYOL, SUBSTR(birth, 1, 6)||'-'||SUBSTR(birth, 7, 7) AS birth, "
    sStr = sStr & "                 HOPE_ACACD, SEX, NVL(KEYOL,'1') AS KEYOL, SUBSTR(BIRTH, 1, 4)||'.'||SUBSTR(BIRTH, 5, 2)||'.'||SUBSTR(BIRTH, 7, 2) AS birth, "
    sStr = sStr & "                 SEL1, SEL2, SEL3, SEL4, SEL5, PTS_SEL, PTS1, PTS2, "
    sStr = sStr & "                 SCORE1, SCORE2, SCORE3, T_RANK, ALL_CNT, 0 AS GTOT,"
    ...
    sStr = sStr & "           WHERE EXMROUND LIKE 'KN081126%'"
    ...
    With DBRec
        If .RecordCount > 0 Then
            ...
            For nRec = 1 To .RecordCount Step 1
                ...
                If IsNull(.Fields("SCORE1")) = False Then uSTD(nRec).K_LEV = .Fields("SCORE1")
                If IsNull(.Fields("SCORE2")) = False Then uSTD(nRec).M_LEV = .Fields("SCORE2")
                If IsNull(.Fields("SCORE3")) = False Then uSTD(nRec).E_LEV = .Fields("SCORE3")
                
                If IsNull(.Fields("T_RANK")) = False Then uSTD(nRec).T_RANK = .Fields("T_RANK")
                If IsNull(.Fields("ALL_CNT")) = False Then uSTD(nRec).ALL_CNT = .Fields("ALL_CNT")
...
Private Sub Std_Data_Show(Index As Long)
    ...
    With uSTD(Index)
        ...
        sTmp = sTmp & "언어(" & Trim(.K_LEV) & "), "
        sTmp = sTmp & "수학(" & Trim(.M_LEV) & "), "
        sTmp = sTmp & "영어(" & Trim(.E_LEV) & ")"
        학교석차.Text = "등급 : " & sTmp & "   석차 : " & Trim(.T_RANK) & "/" & Trim(.ALL_CNT)
...
ErrPrint:
    ...
    '****************************************************************************************
    pRate = 0.9             ' 1.15
    basFunction.PrintStartDoc pReportViewer.Width * pRate, pReportViewer.Height * pRate, vbPRPSB5, vbPRORLandscape, 1, 1            'vbPRPSA4
