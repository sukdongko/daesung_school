...
Begin VB.Form STD011_N 
   Caption         =   "입학사정 >> 학생전체 조회(노량진,송파)"
   ClientHeight    =   10890
   ClientLeft      =   3360
   ClientTop       =   1935
   ClientWidth     =   15360
   ...
   Begin VB.Frame Frame18 
      ...
      Begin VB.Frame Frame19 
         ...
         Begin FPSpread.vaSpread sprSTD_F 
            ...
            SpreadDesigner  =   "STD011_N.frx":0000
         End
   ...
   Begin VB.Frame Frame20 
      ...
      Begin VB.Frame Frame21 
         ...
         Begin FPSpread.vaSpread sprExcel_STD_Data 
            ...
            EndProperty
            MaxCols         =   16
            SpreadDesigner  =   "STD011_N.frx":303B
         End
   ...
   Begin FPSpread.vaSpread sprStdData 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      SpreadDesigner  =   "STD011_N.frx":C3A4
   End
End
Attribute VB_Name = "STD011_N"
...
Private Sub cmdFind_Click()
    ...
    Select Case Trim(basModule.SchCD)
        ...
        Case "P"
            sStr = sStr & " DECODE(PTS_SEL,'8','수능','9','2010 평가','6','3등급','','없음') AS PTS_SEL "
        Case Else
    ...
    Select Case Trim(Right(cboPassCN, 30))
        Case "ALL"      ' /* 합격생은 모두 조회 */
            ...
            sStr = sStr & "              MAX(REGDATE) AS REGDATE, MAX(PAYGBN) AS PAYGBN, MAX(PTS_SEL) AS PTS_SEL, MAX(MU_TYPE) AS MU_TYPE "
            ' 2010.12.13 김한욱, 학생 조회 시 점수가 더블로 나와서 SUM을 MAX로 변환
                    sStr = sStr & " , "
            ...
            sStr = sStr & "                      TO_CHAR(A.REGDATE,'YYYY-MM-DD') AS REGDATE, GET_PAYGUBN(A.ORD_NO) AS PAYGBN, PTS_SEL, MU_TYPE "
            ...
            '<< 등록일자 >>
            If Trim(fpDay1.UnFmtText) <> "" And Trim(fpDay2.UnFmtText) <> "" Then
                sStr = sStr & "              AND REGDATE BETWEEN TO_DATE('" & Trim(fpDay1.UnFmtText) & "000000', 'YYYYMMDDHH24MISS') "
                sStr = sStr & "                              AND TO_DATE('" & Trim(fpDay2.UnFmtText) & "235959', 'YYYYMMDDHH24MISS') "
            ElseIf Trim(fpDay1.UnFmtText) = "" And Trim(fpDay2.UnFmtText) <> "" Then
        ...
        Case Else       ' /* 특정 합격차수의 합격자만 조회함 */
            sStr = sStr & " FROM ("
                    ...
                    ElseIf Trim(fpDay1.UnFmtText) <> "" And Trim(fpDay2.UnFmtText) = "" Then
                        sStr = sStr & "      AND REGDATE BETWEEN TO_DATE('" & Trim(fpDay1.UnFmtText) & "000000', 'YYYYMMDDHH24MISS') "
                        sStr = sStr & "                      AND TO_DATE('99991231235959', 'YYYYMM  DDHH24MISS') "
                    End If
...
'>> 엑셀 DB Open
    ...
    If DBExRec.RecordCount = 0 Then
        ...
        MsgBox "Excel Data가 없습니다.", vbExclamation + vbOKOnly, "IT2010"
        Exit Sub
    End If
...
'    sStr = sStr & "         ORD_NO AS 주문번호, CASH_BILL_NUM AS 현금영수증"
...
'    Select Case Trim(basModule.SchCD)
'        Case "S"
'            'sStr = sStr & " DECODE(PTS_SEL,'1','수능등급','2','6월 평가원','3','9월 평가원','4','6월 평가원','5','9월 평가원','') AS 구분, "
...
'    sStr = sStr & "         DECODE(MU_TYPE,'1','수능등급','2','6월 평가원','3','9월 평가원','4','6월 평가원','9','내신등급','5','9월 평가원','수능') AS 등급, "
...
'
'        ' 클리닉 추가
'        sStr = sStr & AddSQL_ClinicToExcel()
'
'    sStr = sStr & "    FROM ( "
'
...
'            sStr = sStr & "         MAX(SEL1      ) AS SEL1       , MAX(SEL2      ) AS SEL2       , MAX(SEL3      ) AS SEL3      , MAX(SEL4      ) AS SEL4      , MAX(SEL5      ) AS  SEL5     , MAX(SEL7      ) AS  SEL7      ,"
...
Private Sub imgExcel_Click()
    ...
    '학생 엑셀 저장 쿼리문
    sStr = basCommonSTD.Get_StdExcuteSqlToExcel_N(cboKaeyol_F.Text, fpDay1.UnFmtText, fpDay2.UnFmtText)
