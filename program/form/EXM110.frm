...
Private Sub cmdFind_Click()
    Call inits
    Call Find_Monthly                   '<< 점수자료 조회
    
    If nTotRec > 0 Then
        Call Disp_STD_JumsuData(1)      '<< 학생점수 자료 disp
        
    Else
        MsgBox "시험성적 내용이 없습니다.", vbExclamation + vbOKOnly, "학생점수 데이터"
    End If
...
Private Sub Disp_STD_JumsuData(ByVal aindex As Long)
    ...
    sBoldYM = Format(Now, "yyyy-mm")            '< 현재월 기준
    ...
    '2011-05-19 김한욱 노량진 대성 신현우씨 요청으로 월요일에 해당하는 데이터만 중앙에 표기
    
    nPosition = 3
    
    For nDay = 1 To 31 Step 1
        For nDayText = nPosition To 23 Step 5
            If Mid(uSTD(aindex).M1D(nDay), 7, 10) = "(월)" Then
    ...
    For nDay = 1 To 31 Step 1
        For nDayText = nPosition To 23 Step 5
            If Mid(uSTD(aindex).M2D(nDay), 7, 10) = "(월)" Then
    ...
    For nDay = 1 To 31 Step 1
        For nDayText = nPosition To 23 Step 5
            If Mid(uSTD(aindex).M3D(nDay), 7, 10) = "(월)" Then
    ...
    For nDay = 1 To 31 Step 1
        For nDayText = nPosition To 23 Step 5
            If Mid(uSTD(aindex).M4D(nDay), 7, 10) = "(월)" Then
    ...
    For nDay = 1 To 31 Step 1
        For nDayText = nPosition To 23 Step 5
            If Mid(uSTD(aindex).M5D(nDay), 7, 10) = "(월)" Then
...
Private Sub Find_Monthly()
    ...
    With DBRec
        ...
        If .RecordCount > 0 Then
            ...
            ReDim uSTD(.RecordCount) As tSTD                '<< 학생수 만큼 레코드 만듬.
